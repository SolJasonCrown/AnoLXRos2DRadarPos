...
LOCAL_MODE = os.environ.get("FISHROS_LOCAL", "0") == "1"
...
CONFIG_TOOL = 2  # 配置相关
INSTALL_AI = 3  # AI相关
INSTALL_AUTOINSTALL = 4  # Autoinstall扩展
...
tools_type_map = {
    ...
    CONFIG_TOOL: "配置工具",
    INSTALL_AUTOINSTALL: "Autoinstall扩展",
}
...
tools = {
    ...
    19: {
        "tip": "扩展工具箱:ROS/雷达/SLAM/视觉/系统工具(迁移自Autoinstall.sh)",
        "type": INSTALL_AUTOINSTALL,
        "tool": "tools/tool_install_autoinstall_suite.py",
        # 该扩展内部会直接调用FishROS原生ROS/Cartographer工具。
        # 本地模式下所有tools文件已存在；远程模式下将这些基础工具一并下载。
        "dep": [1, 3, 4, 5, 9],
    },
    # 19: {
...
def main():
    ...
    if tr.country != "CN" and not LOCAL_MODE:
...
更多工具教程，请访问鱼香ROS官方网站:http://fishros.com
    ...
    if code == 0:
        ...
        if url_prefix and not LOCAL_MODE:
